        ...
        try (PrintWriter out = response.getWriter()) {
              ...
              try
            {
                ...
                com.mysql.jdbc.Connection con=(com.mysql.jdbc.Connection) DriverManager.getConnection("jdbc:mysql://localhost/hostel","root","");  
                     ...
                     PreparedStatement stmt1=con.prepareStatement("update registerug2 set contact=? where appno=?");
                     stmt1.setString(1, no);
                     stmt1.setString(2, MyGlobals.Gname);
                     stmt1.executeUpdate();
                     PreparedStatement stmt2=con.prepareStatement("update registerug2 set email=? where appno=?");
                     stmt2.setString(1, mail);
                     stmt2.setString(2, MyGlobals.Gname);
                     stmt2.executeUpdate();
                     out.println("<script>alert('Details Updated');</script>");
                     request.getRequestDispatcher("updateacc").include(request, response);
