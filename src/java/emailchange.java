...
public class emailchange extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet emailchange</title>");            
            out.println("</head>");
            out.println("<body>");
            try
            {
                ...
                com.mysql.jdbc.Connection con=(com.mysql.jdbc.Connection) DriverManager.getConnection("jdbc:mysql://localhost/hostel","root","");  
                     ...
                     PreparedStatement stmt1=con.prepareStatement("update parent set pmail=? where appno=?");
                     stmt1.setString(1, now);
                     stmt1.setString(2, MyGlobals.Gname);
                     
                     stmt1.executeUpdate();
                     out.println("<script>alert('Your Email ID Updated');</script>");
                   request.getRequestDispatcher("changeemail.html").include(request, response);
